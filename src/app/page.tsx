        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 md:px-8 pt-20 pb-24 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="hidden lg:block space-y-3">
            {featured.slice(0, 3).map((biz) => (
              <Link key={biz.slug} href={`/entreprise/${biz.slug}`}>
                <div className="glass-panel rounded-xl p-4 border border-white/10 hover:border-primary/30 transition-all group cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-primary/20 flex items-center justify-center flex-shrink-0">
                      <span className="font-headline font-black text-primary-container">{biz.name.charAt(0)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-headline font-bold text-white text-sm truncate group-hover:text-primary-container transition-colors">{biz.name}</p>
                      <p className="text-xs text-white/60 truncate">{biz.commune} · {biz.category}</p>
                    </div>
